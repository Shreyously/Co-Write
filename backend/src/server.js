...
ioServer.on('connection', (socket) => {
    ...
    socket.on('get-document', async (documentId, title) => {
        try {
            ...
            // Broadcast cursor / selection activity in the editor
            socket.on('cursor-activity', (range) => {
                try {
                    if (!range) return;
                    ...
                    socket.broadcast.to(documentId).emit('cursor-activity', payload);
                } catch (error) {
                    console.error('Error handling cursor activity:', error);
                }
            });
